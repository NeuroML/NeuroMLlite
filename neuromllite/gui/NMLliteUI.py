...
class NMLliteUI(QWidget):
    ...
    def add_tab(
        ...
        if figure:
            ...
            if toolbar:
                thisToolbar = NavigationToolbar(thisCanvas, self)
            ...
            if options:
                ...
                if toolbar:
                    thisFigureLayout.addWidget(thisToolbar)
            else:
    ...
    def __init__(self, nml_sim_file, parent=None):
        ...
        offset_opt = 1
        rasterOptionsLayout = self.add_tab(
            self.SPIKES_RASTERPLOT,
            self.plotTabs,
            figure=True,
            toolbar=True,
            options=True,
        ...
        spikeStatOptionsLayout = self.add_tab(
            self.SPIKES_POP_RATE_AVE,
            self.plotTabs,
            figure=True,
            toolbar=True,
            options=True,
    ...
    def showLemsView(self):
        ...
        lems_file_name = generate_and_run(
            self.simulation,
        ...
        run_jneuroml("", lems_file_name, post_args, verbose=True)
    ...
    def traceSelect(self):
        ...
        for key in sorted(self.current_traces.keys()):
            if key != "t":
                ...
                self.all_cbs[key].stateChanged.connect(
                    partial(self.traceSelectClicked, key)
                )
                color_button = QPushButton("%s" % self.current_traces_colours[key])
                style = (
                    "QPushButton {background-color: %s; color: black;}"
                    % self.current_traces_colours[key]
                )
    ...
    def replotSimResults(self):
        ...
        for i in range(len(xs)):
            ax_traces.plot(
                xs[i], ys[i], label=labels[i], linewidth=0.5, color=colors[i]
            )
        ...
        if self.simulation.plots2D is not None:
            ...
            for plot2D in self.simulation.plots2D:
                ...
                for a in x_axes:
                    ...
                    print_(
                        "Plotting %s for %s in %s: %s" % (a, plot2D, fig, info),
                        self.verbose,
                    )
                    ...
                    if x_axis in self.current_traces.keys():
                        ...
                        xs = self._eval_at_all(
                            x_axis, self.network.parameters, self.current_traces
                        )
                    if y_axis in self.current_traces.keys():
                        ys = self.current_traces[y_axis]
                    else:
                        ys = self._eval_at_all(
                            y_axis, self.network.parameters, self.current_traces
                        )
        ...
        pops_to_use = self._get_sorted_population_ids(
            include_input_pops=include_input_pops
        )
        ...
        for pop_id in sorted(ids_for_pop.keys()):
            ...
            if pop.size is not None:
                ...
                max_id_here = (
                    max(ids_for_pop[pop_id]) if len(ids_for_pop[pop_id]) > 0 else 0
                )

            print_(
                "Finished with spikes for %s, go from %i with max %i"
                % (pop_id, max_id, max_id_here),
                self.verbose,
            )
            max_id += max_id_here
        ...
        for k in self.current_events.keys():
            ...
            if pop_id in pops_to_use:
                ...
                print_(
                    "%s: %s[%s] has %s spikes, so %s Hz"
                    % (k, pop_id, cell_id, len(spikes), rate),
                    self.verbose,
                )
        ...
        print_v(
            "Calculated rates: %s; means: %s; stds: %s" % (rates, avg_rates, sd_rates)
        )
        ...
        ax_pop_rate.set_xticklabels(
            ax_pop_rate.get_xticklabels(), rotation=45, horizontalalignment="right"
        )
