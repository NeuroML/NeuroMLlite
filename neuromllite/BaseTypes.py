...
class Base(object):
    ...
    def __init__(self, **kwargs):
        ...
        for name, value in kwargs.items():
            if verbose:
                print_v(" >  - Init of %s:  %s = %s" % (self.get_type(), name, value))
            ...
            if name in self.allowed_fields:
                ...
                info = "Error, cannot set %s = %s in %s. Allowed fields here:" % (
                    name,
                    value,
                    self.get_type(),
                )
    ...
    def _is_evaluable_expression(cls, value):
        ...
        return (
            ...
            or value.__name__ == "neuromllite.EvaluableExpression"
        )
    ...
    def __setattr__(self, name, value):
        ...
        if verbose:
            print_v(" > Setting attr %s=%s..." % (name, value))
    ...
    def _set_field(self, name, value):
        ...
        if self._is_evaluable_expression(self.allowed_fields[name][1]):
            ...
            if self._is_base_type(
                ...
                raise Exception(
                    "Cannot set field %s to %s. Expecting %s not %s"
                    % (name, value, self.allowed_fields[name][1], type(value))
                )
        ...
        elif self._is_base_type(
            self.allowed_fields[name][1],
            can_be_list=False,
            can_be_dict=False,
            can_be_eval_expr=False,
        ):
                            ...
                            if verbose:
                                print(
                                    ...
                                    % (
                                        ...
                                        type(var.fields[field_name]),
                                    )
                                )
                            d[field_name] = cls.to_dict_format(
                                var.fields[field_name], ordered=ordered
                            )
    ...
    def generate_documentation(self, format=MARKDOWN_FORMAT):
        ...
        for f in self.allowed_fields:
            referencable = not Base._is_base_type(
                self.allowed_fields[f][1], can_be_eval_expr=True, can_be_dict=True
            )
        ...
        for c in self.allowed_children:
            ...
            referencable = not Base._is_base_type(
                self.allowed_children[c][1], can_be_dict=True
            )
